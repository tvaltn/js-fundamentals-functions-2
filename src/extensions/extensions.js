...
function estimatePrepTime(ingredients, prepTime) {
  if (prepTime) return ingredients.length * prepTime
  else return ingredients.length * 2
}
...
function calculateQuantities(ingredients, layers) {
  ...
  if (ingredients.includes('sugar')) quantities.sugar = layers * 100
  if (ingredients.includes('eggs')) quantities.eggs = layers * 2
  return quantities
}
...
function improveRecipe(quantities, portions) {
  ...
  Object.keys(quantities).forEach(
    (ingredient) =>
      (newQuantities[ingredient] = quantities[ingredient] * portions)
  )
